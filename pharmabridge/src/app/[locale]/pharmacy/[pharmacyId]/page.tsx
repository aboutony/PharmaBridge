      ...
      <main id="main-content" className="section-shell px-4 py-10">
        <div className="space-y-6">
          ...
          <div className="grid gap-4 tablet:grid-cols-3">
            <Link href={`/${currentLocale}/marketplace`}><GlassPanel level={1} interactive className="p-6"><p className="font-semibold text-white">{currentLocale === 'ar' ? 'العودة إلى السوق' : 'Back to marketplace'}</p></GlassPanel></Link>
            <Link href={`/${currentLocale}/patient/dashboard`}><GlassPanel level={1} interactive className="p-6"><p className="font-semibold text-white">{currentLocale === 'ar' ? 'طلبات المريض' : 'Patient orders'}</p></GlassPanel></Link>
            <Link href={`/${currentLocale}/pharmacy/dashboard`}><GlassPanel level={1} interactive className="p-6"><p className="font-semibold text-white">{currentLocale === 'ar' ? 'لوحة الصيدلية' : 'Pharmacy operations'}</p></GlassPanel></Link>
          </div>
        </div>
      </main>
    </PlatformShell>
